...
**Status 2026-08-25: D0 is DONE** — `scripts/build_v3.py` ran; balanced file probes: register-only
**0.536**, length-only **0.536**, single-label share 0.811 → **0.0** (116,775 docs,
`artifacts/MANIFEST.json` → `v3_build`). The paragraph below records why it was needed.
...
**Free AI-side volume first: the Nemotron 3 Ultra release** (CITED, checked 2026-08-25). NVIDIA
shipped Nemotron 3 Ultra's training data under permissive terms (OpenMDW-1.1 / CC-BY mix, some
subsets gated behind an HF approval click). Two parts fit this project: the Nemotron-CC synthetic
web *rephrasings* (LLM-rewritten web pages — the `rewrite` method in our register, at enormous
scale) and the post-training responses, which add generator families we lack (Qwen3-235B,
DeepSeek-V3/V4-Pro, GPT-OSS-120B, Nemotron itself), each recorded per dataset — ideal for
unseen-generator holdouts. Sample ~40–60k docs. **Asymmetric-trust rule:** their synthetic side is
safely AI-labeled; their "organic" side spans 2013–2025 and post-2022 crawl contains wild AI text,
so it is never used as human training data unless dated pre-2022. This shrinks the paid campaign to
the methods no release ships: `continue`, `polish`, `antislop`, `humanize`.
